        ...
        query.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists() && snapshot.getChildrenCount()>0) {
                    ...
                    totalAmountSpendOn.setText("Total day's spending: $ "+ totalAmount);
                    monthSpendAmount.setText("Total spent: $ "+totalAmount);
                }
                else {
                    totalAmountSpendOn.setText("You've not spent today");
        ...
        personalRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    Toast.makeText(DailyAnalyticsActivity.this, "Child dose not exist", Toast.LENGTH_SHORT).show();
                }
            }
        ...
        personalRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    float appTotal;
                    if (snapshot.hasChild("dayCha")){
                        appTotal = Integer.parseInt(snapshot.child("dayCha").getValue().toString());
                    } else {
                        appTotal = 0;
                    }
